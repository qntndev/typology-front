...
class EnumTypeComposer {
  ...
  static createTemp(typeDef, schemaComposer) {
    ...
    if ((0, _is.isString)(typeDef)) {
      ...
      if (_TypeMapper.TypeMapper.isTypeNameString(typeName)) {
  ...
  constructor(graphqlType, schemaComposer) {
    ...
    this._gqType.getValues().forEach((_ref) => {
      let {
        name,
        isDeprecated
      } = _ref,
          config = _objectWithoutProperties(_ref, ["name", "isDeprecated"]);

      this._gqcFields[name] = _objectSpread({
        extensions: {}
      }, config);
  ...
  getType() {
    if (_graphqlVersion.graphqlVersion >= 14) {
      this._gqType._values = (0, _configToDefine.defineEnumValues)(this._gqType, this._gqcFields);
